...
enum instat_t {norm, plus, minus, leer, punkt, match, exakt, server};
instat_t instat;
uint16_t inp;
bool inpAkt = false;
...
bool verbo = false;
bool servMode = false; // true stellt aufgabe
bool autoMode = true;   // true search after cr
byte showMode = 10;  // 0: show results per page, else in blocks of (here) 10, switch with m
byte showCnt;
...
void prompt() {
  ...
  if (servMode) {
    ...
    if (autoMode) Serial.print("A>"); else Serial.print("N>");
  }
}

void help() {
  prlnF(F("Ein Wort mit 5 Buchstaben in wordl eingeben, dann"));
  prlnF(F("die Buchstaben und angezeigten Farben hier eingeben:"));
  prlnF(F("grau:  -  nachfolgende Buchstaben duerfen nicht vorkommen"));
  prlnF(F("gelb:  .  nachfolgende Buchstaben muessen, aber nicht hier vorkommen"));
  prlnF(F("gruen: #  nachfolgende Buchstaben muessen genau hier stehen"));
  prlnF(F("Beispiel: -a.s#i-en  meint  a grau, s gelb, i gruen, e und n grau"));
  prlnF(F("s (show) zeigt fuer jeden Buchstaben die nicht erlaubten Positionen,"));
  prlnF(F("         den aktuelle Filter und die Muss-Buchstaben"));
  prlnF(F("i (init) auf Anfang zuruecksetzen"));
  prlnF(F("a (auto) mode umschalten, wenn an dann suche nach CR"));
  prlnF(F("l (loop) einmal suchen"));
  prlnF(F("z (zaehle) Buchstaben, Anzeige absolut und prozentual"));
  prlnF(F("Zum Debuggen:"));
  prlnF(F("+ nachfolgende Buchstaben muessen da sein"));
  prlnF(F("\\ nachfolgende Buchstaben Restriktion loeschen"));
  prlnF(F("=*****  Filter fuer gruene Buchstaben"));
  prlnF(F("v (verbose) mode umschalten, wenn an dann genauere Infos"));
  prlnF(F("nnnb Block nnn einlesen und zeigen,  z.B. 4b"));
  prlnF(F("c    diesen Block zeigen"));
  prlnF(F("p    diesen Block pruefen"));
  prlnF(F("nnnm Anzahl Ergebnisse pro Zeile, 0m eine Zeile pro Block"));
...
void doCmd(byte tmp) {
  ...
  switch (tmp) {
    case 'a':   //
      autoMode = !autoMode;
      prnF(F("utom "));
      if (autoMode) prlnF(F("an")); else  prlnF(F("aus"));
      break;
    case 'b':   //
      copyBeis(inp);
      showBuff();
    ...
    case 'z':   //
      ...
      break;
    default:
      msgF(F("??  -a.b#c, +, =, \\, 0..9, init, loop, verbose, autom, show, "), tmp);
      prlnF(F("help pruf, zahl, nnnblock, nnmode"));
  } //case
  prompt();
}
